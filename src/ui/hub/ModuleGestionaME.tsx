      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-[#050811] border border-white/5 rounded-[3rem] p-10 relative overflow-hidden shadow-2xl">
          <div className="flex justify-between items-start mb-8">
            <div>
              <h3 className="text-emerald-400 font-black text-xs uppercase tracking-[0.4em] mb-2">{t('ui.erp_sync')}</h3>
              <p className="text-slate-400 text-[10px] font-mono">{t('ui.as400_gateway')}</p>
            </div>
          ...
          <div className="space-y-4">
            <h4 className="text-white text-[10px] font-black uppercase tracking-widest mb-4">{t('ui.tactical_queue')}</h4>
            {[
              ...
              <div key={task.id} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-emerald-400/30 transition-all group">
                <div className="flex items-center gap-4">
                  <span className="text-[10px] font-mono text-emerald-400">{task.id}</span>
                  <span className="text-xs text-slate-300 font-medium">{task.task}</span>
                </div>
        ...
        <div className="bg-[#0a0f1d] border border-white/5 rounded-[3rem] p-10 shadow-xl">
          <h3 className="text-white font-black text-xs uppercase tracking-widest mb-8 flex items-center gap-3">
            <Network size={16} className="text-emerald-400"/> {t('ui.system_health')}
          </h3>
          <div className="space-y-8">
            <div className="relative h-40 w-full bg-black/40 rounded-3xl border border-white/5 p-6 flex items-center justify-center">
              <div className="text-center">
                <Activity size={48} className="text-emerald-400 mx-auto mb-4 opacity-20" />
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{t('ui.real_time_traffic')}</p>
                <p className="text-white font-black text-2xl mt-1">98.4%</p>
              </div>
            </div>
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-[10px] text-slate-500 font-bold uppercase">{t('ui.cpu_load')}</span>
                <span className="text-[10px] text-white font-mono">12%</span>
              </div>
              <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                <div className="h-full bg-emerald-400 w-[12%]" />
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[10px] text-slate-500 font-bold uppercase">{t('ui.memory_usage')}</span>
                <span className="text-[10px] text-white font-mono">2.4GB</span>
              </div>
              <div className="h-1 bg-white/5 rounded-full overflow-hidden">
