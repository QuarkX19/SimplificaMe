/**
 * SIMPLIFICAME – AS/400 KPI Sync Service
 * Conecta AS/400 vía IWS → sincroniza KPIs en Supabase
 * Alimenta automáticamente L2, L4 y L7
 *
 * ✅ FIX: onConflict ahora usa 'company_id,cycle_id,name' — requiere el índice:
 *    CREATE UNIQUE INDEX IF NOT EXISTS uq_kpis_company_cycle_name
 *    ON kpis(company_id, cycle_id, name);
 * ✅ FIX: retry automático ante fallos transitorios del IWS
 * ✅ FIX: validación de payload antes de upsert
 * ✅ FIX: erp_connections.connection_id incluido en el log
 */
...
// ── Sincronizar KPIs desde AS/400 vía IWS ──
export const syncAS400Kpis = async (
  companyId: string,
  cycleId: string,
  iwsEndpoint: string,
  connectionId?: string  // ✅ para linkear en erp_sync_log
  ...
  try {
    const response = await fetchWithRetry(iwsEndpoint); // ✅ reintentos automáticos
    const rawData = await response.json();

    // ✅ Validar que llegó algo parseable
    ...
    if (kpis.length === 0) {
      throw new Error('mapAS400ToKpis no generó KPIs — revisa los nombres de campos del AS/400');
    }

    for (const kpi of kpis) {
      // ✅ Validar cada KPI antes de enviarlo
      ...
      const { error } = await supabase.from('kpis').upsert(
        ...
        // ✅ FIX CRÍTICO: usar los 3 campos que forman el índice único real
    ...
    await supabase.from('erp_sync_log').insert({
      company_id:     companyId,
      connection_id:  connectionId ?? null, // ✅ trazabilidad
...
export const getLayerKpis = async (
  ...
  // ✅ Filtrar por ciclo si se pasa — evita mezclar KPIs de ciclos viejos
  if (cycleId) query = query.eq('cycle_id', cycleId);

  const { data, error } = await query;
  if (error) {
    console.error('[AS400] getLayerKpis error:', error.message);
    return [];
  }
  return data;
};
